		...
		const unsigned char *message, const int msglen, const int64 *pubkey, int k)
{
	...
	cudaMalloc(&d_y1, (PASS_N * sizeof(int64))*k);
	cudaMalloc(&d_Fy1, (PASS_N * sizeof(int64))*k);
	cudaMalloc(&d_y2, (PASS_N * sizeof(int64))*k);
	cudaMalloc(&d_Fy2, (PASS_N * sizeof(int64))*k);
	...
	cudaMemcpy(d_y1,y1,(PASS_N * sizeof(int64))*k,cudaMemcpyHostToDevice);
	cudaMemcpy(d_Fy1,Fy1,(PASS_N * sizeof(int64))*k,cudaMemcpyHostToDevice);
	cudaMemcpy(d_y2,y2,(PASS_N * sizeof(int64))*k,cudaMemcpyHostToDevice);
	cudaMemcpy(d_Fy2,Fy2,(PASS_N * sizeof(int64))*k,cudaMemcpyHostToDevice);


	ntt_gpu(d_Fy1, d_y1,k);
	ntt_gpu(d_Fy2, d_y2,k);	// computing P(y1,y2)

	ntt_gpu(d_Fy, d_y,k);

	cudaMemcpy(y,d_y,(PASS_N * sizeof(int64)*k),cudaMemcpyDeviceToHost);
	cudaMemcpy(y1,d_y1,(PASS_N * sizeof(int64)*k),cudaMemcpyDeviceToHost);
	cudaMemcpy(y2,d_y2,(PASS_N * sizeof(int64)*k),cudaMemcpyDeviceToHost);
	...
	int64 d_t[PASS_N];
	b_sparse_poly d_c;
	int counter=0;; //to keep track of rejected signatures

	cudaMalloc(&d_x, (PASS_N * sizeof(int64))*k);
	cudaMalloc(&d_g, (PASS_N * sizeof(int64))*k);
	cudaMalloc(&d_c, (sizeof(b_sparse_poly))*k);
	cudaMalloc(&d_t, (PASS_N * sizeof(int64))*k);
	cudaMalloc(&d_key, (PASS_N * sizeof(int64)));

	cudaMemcpy(d_t,t,(PASS_N * sizeof(int64)*k),cudaMemcpyHostToDevice);
	cudaMemcpy(d_x,x,(PASS_N * sizeof(int64)*k),cudaMemcpyHostToDevice);
	cudaMemcpy(&d_c,&c, sizeof(b_sparse_poly)*k,cudaMemcpyHostToDevice);
	cudaMemcpy(d_g,g,(PASS_N * sizeof(int64))*k,cudaMemcpyHostToDevice);
	cudaMemcpy(d_key,key,(PASS_N * sizeof(int64)),cudaMemcpyHostToDevice);


	circ_conv_gpu(d_g,d_key,pubkey,k);
	bsparseconv_gpu(d_x,d_g,&d_c,k); //generating x = (-1/q)m*g
	...
	circ_conv_gpu(d_t,d_y,pubkey,k); //generating t=s*h

	cudaMemcpy(t,d_t,(PASS_N * sizeof(int64)*k),cudaMemcpyDeviceToHost);
	cudaMemcpy(y,d_y,(PASS_N * sizeof(int64)*k),cudaMemcpyDeviceToHost);
