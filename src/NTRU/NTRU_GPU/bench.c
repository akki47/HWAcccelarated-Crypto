...
main(int argc, char **argv)
{
  ...
  printf("------------------------------------------\n");
  printf("NTRU, SIGNATURE, CPU , random");
  printf("\nParameters: N: %d, p: %d, g: %d, k: %d, b: %d, t: %d\n\n",
        PASS_N, PASS_p, PASS_g, PASS_k, PASS_b, PASS_t);
  printf("------------------------------------------\n");
  printf("\n#msg \t Latency(us)\t Throughput(msg/sec)\t Total Sigs generated\n");

  clock_t c0,c1;

    for( k=2;k<=8192;k=k*2)
    {
...
//          VERIFY ? "and verifying" : "and not verifying");
 ...
 // printf("Attempts/sig: %f\n",  (((float)count)/k));
  ...
  printf("\n%4d\t\t%.2f\t\t%.2f\t\t%d\n",k,((float)(c1 - c0)),((k*1000000.0/((float)(c1 - c0)))),count);
  }
#if DEBUG
  printf("\n\nKey: ");
  for(i=0; i<PASS_N; i++)
